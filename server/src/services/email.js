...
const wrap = (title, body, color = '#2c3e50') =>
...
export async function sendDueSoon(user, book, loan) {
  const html = wrap(
    'Nhắc trả sách',
    `<p>Chào <b>${user.name}</b>,</p><p>Sách <b>${book.title}</b> sẽ đến hạn trả vào ngày
     <b>${new Date(loan.dueDate).toLocaleDateString('vi-VN')}</b>. Vui lòng trả đúng hạn để tránh bị phạt.</p>`
  );
  await getTransport().sendMail({ to: user.email, subject: `[Thư viện] Nhắc trả sách: ${book.title}`, html });
}

export async function sendOverdue(user, book, loan, days, fine) {
  const html = wrap(
    'Sách đã quá hạn',
